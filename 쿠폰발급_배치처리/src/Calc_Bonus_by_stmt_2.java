...
public class Calc_Bonus_by_stmt_2 {
    public static void main(String[] args) {
        ...
        try {
            ...
            stmt_ins = conn.createStatement(); // 자동 커밋 비활성화

            conn.setAutoCommit(false);

            // 고객 정보를 조회하는 SQL 쿼리
            sql = "SELECT id, email, credit_limit, address1, enroll_dt, gender FROM customer";
            ...
            while (rs.next()) {
                ...
                // 보너스 쿠폰 계산 로직
                if (!enrollDt.before(Date.valueOf("2018-01-01"))) { // 2018-01-01 포함
                    if (creditLimit < 1000) {
                        coupon = "AA";
